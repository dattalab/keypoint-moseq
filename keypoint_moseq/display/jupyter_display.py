...
class JupyterDisplay:
    def start_set_group_labels(self, initial_group_labels: Mapping[str, str], save_group_labels: Callable[[dict[str, str]], None]):
        ...
        def _save_group_labels(n_clicks, table_data):
            ...
            # Empty table case with no recordings
            if len(table_data) == 0:
                group_labels = {}
            # Dash removes group-label column if the user hits 'clear' and doesn't fill in anything
            # afterwards. Handle this case manually.
            # Also, Dash allows users to add rows. The 'if' prunes any extra rows added.
            elif 'group-label' not in table_data[0].keys():
                group_labels = {
                    row['recording-name']: ''
                    for row in table_data
                    if row['recording-name'] in initial_group_labels.keys()
                }
            else:
                group_labels = {
